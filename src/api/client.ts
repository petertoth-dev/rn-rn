...
import ApiRequestError, {
  ApiRequestErrorType,
} from '@src/exceptions/ApiRequestError';
...
export const get = async <T>(
  ...
  if (!netInfo.isConnected) {
    if (cacheMode === 'disabled') {
      // We are offline and caching is disabled
      throw new ApiRequestError(
        599,
        'The device is offline, and cache was disabled for this request: ' +
          url +
      ...
      if (!cachedResponse?.data) {
        throw new ApiRequestError(
          599,
          'The device is offline, and the response is not cached for this request: ' +
            url +
            JSON.stringify(params),
        );
      }
...
export interface GetRequestState<T, M = unknown> {
  ...
  get: (
    ...
    params?: object,
    options?: object,
  ) => Promise<ApiResponse<T, M> | null | undefined>;
  reset: () => void;
  cancel: () => void;
}
...
export const useGetRequest = <T, M = unknown>() => {
  ...
  const getRequest = useCallback(
    async (
      ...
      try {
        const responseData = await get<ApiResponse<T, M>>(url, params, { cancelToken: cancelTokenRef.current?.token, ...options, });
  ...
  const cancel = () => {
    cancelTokenRef.current?.cancel(
      'Request has been cancelled by cancel() method.',
    );
  };
